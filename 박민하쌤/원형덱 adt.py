class Deque:
    ...
    def addFront(self, e):  #front에서 값 삽입 함수
        if not self.isFull():  #가득 차지 않았는가?
            self.front = (self.front - 1 + self.capacity) % self.capacity  #front 값 변경
            self.list[self.front] = e  #현재 rear 위치에 입력받은 값 삽입
        else:  #가득 찼다면
            print("덱이 가득 찼습니다.")  #출력
    ...
    def deleteFront(self):  #front에서 값 제거 함수
        if not self.isEmpty():  #비어있지 않은가?
            ...
            self.front = (self.front + 1) % self.capacity  #rear를 하나 낮춤
            print(item)  #제거 값 출력
    ...
    def addRear(self, e):  #rear에서 값 삽입 함수
        if not self.isFull():  #가득 차지 않았다면
            self.list[self.rear] = e  #현재 front 위치에 입력받은 값 삽입
            self.rear = (self.rear + 1) % self.capacity  #rear값 변경
        else:  #가득찼다면
            print("덱이 가득 찼습니다.")  #출력

    def deleteRear(self):  #rear에서 값 제거 함수
        if not self.isEmpty():  #비어있지 않다면
            self.rear = (self.rear - 1 % self.capacity) % self.capacity  #front 다음 공간 지정
